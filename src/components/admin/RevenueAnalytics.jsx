...
export default function RevenueAnalytics({ dateFilter }) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const formattedCat = Object.keys(catMap).map(cat => ({
            category: cat,
            revenue: catMap[cat],
            percentage: totalRev > 0 ? ((catMap[cat] / totalRev) * 100).toFixed(1) : 0
  ...
  if(loading) return <div className="text-white py-10 text-center animate-pulse">Đang vẽ biểu đồ phân tích...</div>
      ...
      <div className="bg-gradient-to-br from-purple-900/60 to-purple-950/60 backdrop-blur-md p-6 rounded-xl shadow-lg border border-purple-700/50">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <AreaChart data={trendData}>
            ...
            <YAxis stroke="#e9d5ff" tickFormatter={(value) => new Intl.NumberFormat('en', { notation: "compact" }).format(value)} />
            <Tooltip
              contentStyle={{ backgroundColor: "#581c87", border: "1px solid #a855f7", borderRadius: "8px" }}
              labelStyle={{ color: "#fdf4ff" }}
              formatter={(value) => new Intl.NumberFormat("vi-VN", { style: "currency", currency: "VND" }).format(value)} 
            />
            <Area type="monotone" dataKey="revenue" stroke="#ec4899" fillOpacity={1} fill="url(#revenueGradient)" name="Tổng doanh thu" />
          </AreaChart>
        </ResponsiveContainer>
        {trendData.length === 0 && <p className="text-center text-pink-200 mt-2 text-sm">Chưa có dữ liệu doanh thu trong khoảng này.</p>}
      </div>
      ...
      <div className="bg-gradient-to-br from-purple-900/60 to-purple-950/60 backdrop-blur-md p-6 rounded-xl shadow-lg border border-purple-700/50">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={categoryData} layout="vertical" margin={{ left: 20 }}>
            ...
            <XAxis type="number" stroke="#e9d5ff" tickFormatter={(value) => new Intl.NumberFormat('en', { notation: "compact" }).format(value)} />
            <YAxis dataKey="category" type="category" stroke="#e9d5ff" width={100} tick={{fontSize: 12}} />
            <Tooltip
              contentStyle={{ backgroundColor: "#581c87", border: "1px solid #a855f7", borderRadius: "8px" }}
              labelStyle={{ color: "#fdf4ff" }}
              formatter={(value) => new Intl.NumberFormat("vi-VN", { style: "currency", currency: "VND" }).format(value)} 
            />
            <Bar dataKey="revenue" fill="#ec4899" radius={[0, 4, 4, 0]} barSize={20} name="Tổng doanh thu" />
          </BarChart>
        </ResponsiveContainer>
        {categoryData.length === 0 && <p className="text-center text-pink-200 mt-2 text-sm">Chưa có dữ liệu thể loại.</p>}
      </div>
    </div>
  )
}