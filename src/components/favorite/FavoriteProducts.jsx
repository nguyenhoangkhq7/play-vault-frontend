  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (
        ...
        !Array.isArray(favoriteGamesResponse) ||
        favoriteGamesResponse.length === 0
      ) {
        setGames([]);
        setLoading(false);
        return;
      ...
      const gamesWithStatus = favoriteGamesResponse.map((game) => ({
        ...
        thumbnailImage:
          game.thumbnail ||
          "https://placehold.co/400x200/3a1a5e/ffffff?text=No+Image",
        ageRating: game.details?.["age-limit"] || "N/A",
        releaseDate: game.details?.published_date?.$date
          ? new Date(game.details.published_date.$date).toLocaleDateString(
              "vi-VN"
            )
          : "N/A",
      ...
      if (
        ...
        errorMessage =
          "Server không phản hồi đúng định dạng dữ liệu (có thể backend chưa chạy hoặc proxy chưa config).";
      } else if (
        err.message.includes("ERR_CONNECTION_REFUSED") ||
        err.message.includes("Failed to fetch")
      ) {
        errorMessage =
          "Không thể kết nối đến server. Vui lòng kiểm tra backend (port 3001).";
      } else {
      ...
      >
        <div className="flex flex-col items-center text-center">
          ...
          <h2 className="text-2xl font-bold text-white mb-2">
            Danh sách game yêu thích của bạn
          </h2>
        ...
        <div className="flex items-center gap-3 mt-4 md:mt-0 w-full md:w-auto justify-end">
          <div className="flex bg-purple-800/80 rounded-md overflow-hidden border border-purple-700/50 shadow-lg">
            <Button
              ...
              className={`rounded-none px-3 py-2 h-9 hover:bg-purple-700 hover:text-purple-200 ${
                viewMode === "grid"
                ...
                <div className="bg-purple-900/40 border border-purple-700/50 rounded-lg overflow-hidden hover:shadow-lg transition-all duration-200 hover:border-purple-600/70 group">
                  <div
                    ...
                    style={{
                      backgroundImage: `url(${
                        game.thumbnailImage ||
                        "https://placehold.co/400x200/3a1a5e/ffffff?text=Game+Image"
                      })`,
                    }}
                  >
                    ...
                    <div className="absolute bottom-4 left-4 flex items-center space-x-2">
                      ...
                      {game.tags.slice(0, 1).map((tag) => (
                        <span
                          key={tag}
                          className="bg-pink-600/90 text-white text-xs px-2 py-1 rounded-full"
                        >
                          {formatTag(tag)}
                        </span>
                      ))}
                ...
                <div className="flex bg-purple-900/40 border border-purple-700/50 rounded-lg hover:shadow-lg transition-all duration-200 hover:border-purple-600/70 overflow-hidden group">
                  ...
                  <div className="flex-1 p-4 flex justify-between">
                    ...
                    <div className="flex items-center space-x-4">
                      <div className="text-purple-200 font-medium text-right">
                        {game.price}
                      </div>
