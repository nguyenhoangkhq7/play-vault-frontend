...
import { TrendingUp, Filter, ArrowUpDown, Search, ChevronRight } from "lucide-react"
...
export default function GameRevenueBreakdown({ dateFilter }) {
  ...
  if(loading) return <div className="text-white p-6 animate-pulse">Đang tải dữ liệu game...</div>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-2 bg-purple-950/60 px-4 py-2 rounded-lg border border-purple-600/50">
            ...
            >
              <option value="all" className="bg-purple-950">Tất cả thể loại</option>
              {uniqueCategories.map(cat => (
                  <option key={cat} value={cat} className="bg-purple-950">{cat}</option>
              ))}
            </select>
          </div>
          ...
          <div className="flex items-center gap-2 bg-purple-950/60 px-4 py-2 rounded-lg border border-purple-600/50">
            ...
            <select
              ...
              className="bg-transparent text-white text-sm outline-none cursor-pointer"
            >
              <option value="revenue" className="bg-purple-950">Doanh thu cao nhất</option>
              <option value="sales" className="bg-purple-950">Lượt bán nhiều nhất</option>
              <option value="name" className="bg-purple-950">Tên A-Z</option>
            </select>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {displayed.map((game) => (
          ...
          >
            ...
            <h4 className="text-white font-semibold mb-1 text-sm line-clamp-1" title={game.name}>{game.name}</h4>
            <div className="text-lg font-bold text-white">
        ...
        {displayed.length === 0 && <p className="text-pink-200 col-span-4 text-center py-4">Không có dữ liệu trong khoảng thời gian này.</p>}
      </div>
