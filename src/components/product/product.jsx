...
export default function GamesPage() {
  ...
  // --- 3. LOGIC LẤY DANH SÁCH YÊU THÍCH (FIX TOKEN) ---
  useEffect(() => {
    let isMounted = true;

    const fetchUserWishlist = async () => {
      // Chỉ gọi khi đã đăng nhập và có Token
      if (!user || !accessToken) return;

      try {
        // 🔥 Gọi trực tiếp API kèm Header Token
        ...
        const listData = Array.isArray(rawData) ? rawData : rawData?.data || [];

        if (isMounted) {
          // Tạo Set chứa các ID game đã thích
          const ids = new Set(
            listData.map((item) => {
              // Ưu tiên lấy ID từ object game, nếu không có thì lấy gameId
              return item.game?.id || item.gameId || item.id;
            })
          );

          setWishlistIds(ids);
          // console.log("❤️ Wishlist IDs loaded:", ids);
        }
      } catch (error) {
    ...
    // Gọi ngay khi accessToken thay đổi (lần đầu vào trang hoặc F5 xong)
    fetchUserWishlist();

    // Giữ tính năng tự update khi quay lại tab (On Focus)
  ...
  }, [user, accessToken]); // 🔥 Quan trọng: Phải có accessToken trong dependency
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
            <div>
              ...
              <p className="text-xs text-purple-300/80 mt-1">
                Thế giới giải trí vô tận đang chờ đón bạn
              </p>
            </div>
            ...
            <div className="flex gap-2">
              <div className="relative group flex-1">
                ...
                <div className="relative flex items-center gap-3 bg-purple-900/50 border border-purple-800 hover:border-purple-700 rounded-xl px-4 py-3 transition backdrop-blur-sm">
                  ...
                  <input
                    type="text"
                    placeholder="Tìm kiếm game, studio..."
                    value={searchQuery}
              ...
              <button
                ...
                className={`px-4 rounded-xl border transition flex items-center gap-2 ${
                  showFilter || filterParams.minPrice || filterParams.maxPrice
                    ? "bg-pink-600/20 border-pink-500/50 text-pink-300"
                    : "bg-purple-900/50 border-purple-800 text-slate-400 hover:border-purple-600 hover:text-white"
                }`}
              >
                <Filter size={20} />{" "}
                  ...
                  <div className="bg-purple-900/95 border border-purple-700 backdrop-blur-xl rounded-xl p-4">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <button
                        ...
                        className="text-xs text-slate-400 hover:text-pink-400 flex items-center gap-1"
                      >
                        <X size={12} /> Đặt lại
                      </button>
                    </div>
                    <div className="flex items-center gap-3">
                      <input
                        ...
                        className="w-1/2 bg-purple-950/50 border border-purple-700 rounded-lg px-3 py-2 text-sm text-white placeholder-slate-500 focus:border-pink-500 focus:outline-none"
                      />
                    ...
                    <button
                      ...
                      className="w-full mt-3 bg-gradient-to-r from-pink-600 to-purple-600 hover:from-pink-500 hover:to-purple-500 text-white text-sm font-bold py-2 rounded-lg transition shadow-lg"
                    >
                      Áp dụng bộ lọc
                    </button>
                  </div>
        ...
        <section>
          <div className="mb-8">
            <h3 className="text-2xl font-bold text-white">
              {filterParams.keyword
                ? `Kết quả cho "${filterParams.keyword}"`
                : "Danh sách Game"}
            </h3>
          ...
          {loading ? (
            ...
            <div className="text-center text-slate-400 py-12 border border-dashed border-purple-800 rounded-xl bg-purple-900/20">
              Không tìm thấy game nào phù hợp với tiêu chí tìm kiếm.
            </div>
          ) : (
            <>
              {/* --- GRID VIEW --- */}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {games.map((game, idx) => (
                    ...
                    >
                      <Link
                        to={`/product/${game.id}`}
                        className="block h-full relative overflow-hidden rounded-xl border border-purple-800/50 bg-purple-900/50 hover:border-purple-700 transition backdrop-blur-xl flex flex-col hover:shadow-2xl hover:shadow-pink-500/10"
                      >
                        <div className="relative overflow-hidden bg-slate-950 h-48 sm:h-56">
                          <img
                            src={game.thumbnail || game.image || fallbackImage}
                            alt={game.name}
                  ...
                  {games.map((game, idx) => (
                    ...
                    >
                      <motion.div
                        ...
                        transition={{ duration: 0.3, delay: idx * 0.05 }}
                        className="group/row p-4 rounded-xl border border-purple-800/50 bg-purple-900/50 hover:border-purple-600 hover:bg-purple-900/80 transition flex flex-col sm:flex-row items-center gap-4 backdrop-blur-xl relative"
                      >
                        <img
                          src={game.thumbnail || game.image || fallbackImage}
                          alt={game.name}
                          className="w-full sm:w-32 h-48 sm:h-24 rounded-lg object-cover shadow-lg"
                          onError={(e) => (e.currentTarget.src = fallbackImage)}
                        />
                        <div className="flex-1 min-w-0 text-center sm:text-left">
                          <h4 className="text-lg font-bold text-white group-hover/row:text-pink-400 transition truncate">
                            {game.name}
                          </h4>
                          <p className="text-sm text-slate-400 line-clamp-1 mt-1">
                            {game.description || "Mô tả đang cập nhật..."}
                          </p>
                          <div className="flex items-center justify-center sm:justify-start gap-3 mt-2 text-sm">
                            <span className="bg-purple-800/50 px-2 py-0.5 rounded text-xs text-purple-200 border border-purple-700">
